...
def data_callback(i,sampleA):
    ...
    if os.path.exists(TEMP_DIR+"/recording.adc.en-EN-finish"):
        f =open(TEMP_DIR+"/recording.adc.en-EN-finish","r")
        number = f.read().strip()
        f.close()
        adc = TEMP_DIR + "/recording.adc.en-EN." + number

        if os.path.exists(adc+"-result.pkl") :


            if os.path.exists(adc+"-end"):

                return 0, 0 ,list_words,length

            try:#
                file_result = open(adc + "-result.pkl", 'rb')
                object_file = pickle.load(file_result)
                file_result.close()
                line = object_file["result"]
            except:
                return 0, 0 ,list_words,length

            list_words = line.split()
            length = len(list_words)
            Start = object_file["start_time"]
            End = object_file["end_time"]
            if line != cache :
                cache = line
            else:
                length = 0
    ...
    return  Start, End ,list_words,length
...
def clean(tempDir):
    ...
    if os.path.exists(tempDir):
        ...
        subprocess.call(["rm -rf %s/*finish" % tempDir], shell=True)
        subprocess.call(["rm -rf %s/*start" % tempDir], shell=True)
        subprocess.call(["rm -rf %s/*result.pkl" % tempDir], shell=True)
        subprocess.call(["rm -rf %s/*end" % tempDir], shell=True)
serverHost = "i13srv53"
serverPort = 60019
inputFingerPrint  = "en-EU"
inputType         = "audio"
outputFingerPrint = "en-EU"
