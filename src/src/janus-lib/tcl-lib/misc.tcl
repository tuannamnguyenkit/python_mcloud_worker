...
#  Module  :  misc.tcl
#  Date    :  $Id: misc.tcl 3500 2012-05-24 13:59:11Z csaam $
#
#  Remarks :  
#
# ========================================================================
...
proc doParallel { body server serverFinish client } {
  ...
  writeLog stderr "\ndoParallel: waiting for [glob -nocomplain *.INIT] ..."
  while { [llength [glob -nocomplain *.INIT]] != [llength [glob -nocomplain *.END]] } {
    fileAfter $waitSeconds
  }

  if {[catch {set SERVER [doParallelServer]}]} {
    # puts stderr "SERVER-LIST [lsort [glob *.INIT]]"
  ...
  if { $SERVER == $semFile } {
    ...
    file delete $SERVER.END

    writeLog stderr "\ndoParallel: server $SERVER waiting for clients ..."
    while { [llength [glob -nocomplain *.END]] } { fileAfter $waitSeconds }
    writeLog stderr "\ndoParallel: server $SERVER finishing up ..."

    file delete $SERVER.INIT

    uplevel $serverFinish

    file delete $SERVER.SERVER
...
proc putsTag { tag args } {
    global PutsArgs

    set PutsArgs $args
    set res      [putsType putsTag $tag]
    unset PutsArgs

    return $res
}

proc putsType { type {tag ""} } {
    global PutsArgs
