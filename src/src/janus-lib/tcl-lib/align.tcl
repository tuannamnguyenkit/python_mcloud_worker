...
proc rawAlign args {
  ...
  for { set x 2 } { $x < $argN } { incr x } {
    ...
    if [string match [lindex $args $x] "-del"] { incr x; unset insp; upvar \#0 [lindex $args $x] insp }
    if [string match [lindex $args $x] "-ins"] { incr x; unset delp; upvar \#0 [lindex $args $x] delp }
  }
  set hyN [llength $hy]
  set coN [llength $co]
