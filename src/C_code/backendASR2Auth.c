...
static void printUsage (char *prgName) {
  ...
  printf ("\nDESCRIPTION\n"
      "\tThis is a example implementation of an ASR backend which connects to"
...
int main (int argc, char * argv[]) {
  ...
  */
  const char *inputFingerPrint  = "en-DB";      /* special finger print for testing */
  const char *inputType         = "audio";
  const char *outputFingerPrint = "en-DB";      /* special finger print for testing */
  ...
  /* connect and process */
  if ( (cloudP = mcloudCreate2 ("asr", MCloudModeWorker, username, password)) == NULL ) {
  ...
  mcloudAddService (cloudP, "MTEC asr", "asr", inputFingerPrint, inputType, outputFingerPrint, outputType, NULL);
